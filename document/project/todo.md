...
### プロフィール画像機能の残作業（2025/01/14 実装中）
- [ ] 質問一覧・詳細画面でのプロフィール画像表示
  - [ ] 質問詳細ページ（`/questions/[id]/page.tsx`）のユーザーアバター更新
  - [ ] 質問一覧ページでのアバター表示更新
  - [ ] プロジェクト詳細ページでのメンバーアバター更新
- [ ] APIレスポンスにプロフィール画像情報を含める
  - [ ] 質問関連APIでユーザー情報にprofileImageを含める
  - [ ] プロジェクトメンバー一覧APIでプロフィール画像を含める
- [ ] プロフィール画像アップロードAPIのエラー解決
  - [ ] Prisma型定義の更新確認
  - [ ] profileImageリレーションのエラー解決
- [ ] エラーハンドリングの改善（Claudeレビューより）
  - [ ] UserSettingsFormのアラートを既存のErrorMessageコンポーネントに置き換える
  - [ ] 画像アップロード/削除のエラーハンドリング強化
